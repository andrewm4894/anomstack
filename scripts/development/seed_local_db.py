                        ...
                        data.append(
                            {
                                ...
                                "metric_value": round(
                                    rate, 6
                                ),  # Exchange rates need more precision
                            }
                        )
...
def generate_llmalerts_data(
    metrics_df: pd.DataFrame, scores_df: pd.DataFrame, llm_alert_rate: float = 0.03
) -> pd.DataFrame:
...
def generate_change_data(
    metrics_df: pd.DataFrame, target_change_rate: float = 0.02
) -> pd.DataFrame:
...
def generate_tholdalerts_data(
    metrics_df: pd.DataFrame, target_tholdalert_rate: float = 0.03
) -> pd.DataFrame:
    ...
    if num_alerts > 0 and total_metrics > 0:
        ...
        for _, row in sample_rows.iterrows():
            ...
            if random.random() < 0.5:
                ...
                breach_explanation = (
                    f"Value {metric_value:.2f} exceeded upper threshold {threshold_value:.2f}"
                )
            else:
                ...
                breach_explanation = (
                    f"Value {metric_value:.2f} fell below lower threshold {threshold_value:.2f}"
                )
...
def main():
    ...
    parser.add_argument(
        "--hours", type=int, default=72, help="Hours of historical data to generate (default: 72)"
    )
    parser.add_argument(
        "--interval",
        type=int,
    ...
    parser.add_argument(
        "--force", action="store_true", help="Remove existing database if it exists"
    )
    parser.add_argument(
        "--alert-rate",
        type=float,
    ...
    print(
        f"   Alert rates: {args.alert_rate * 100:.1f}% alerts, {args.llm_alert_rate * 100:.1f}% LLM, {args.threshold_alert_rate * 100:.1f}% threshold, {args.change_rate * 100:.1f}% changes"
    )
    print()
    ...
    for metric_batch in metric_batches:
        ...
        if dataframes:
            ...
            try:
                ...
                # Add indexes
                conn.execute(
                    f"CREATE INDEX idx_{table_name}_timestamp ON {table_name}(metric_timestamp)"
                )
                conn.execute(
                    f"CREATE INDEX idx_{table_name}_batch_name ON {table_name}(metric_batch, metric_name)"
                )
                ...
                for _, stat_row in stats_df.iterrows():
                    print(
                        f"      {stat_row['metric_type']}: {stat_row['total_rows']} rows, {stat_row['unique_metrics']} unique metrics"
                    )
    ...
    print("💡 To use this database with the dashboard:")
