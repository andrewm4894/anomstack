...
def detect_threshold_alerts(df: pd.DataFrame, thresholds: dict, recent_n: int = 1, snooze_n: int = 3) -> pd.DataFrame:
    """
    ...
    for metric_name in df['metric_name'].unique():
        ...
        for _, row in recent_observations.iterrows():
            ...
            mask = (df['metric_name'] == metric_name) & (df['metric_timestamp'] == row['metric_timestamp'])
            ...
            if 'upper' in metric_thresholds and metric_value > metric_thresholds['upper']:
                ...
                logger.info(f"Upper threshold alert for {metric_name}: {metric_value} > {metric_thresholds['upper']}")
                ...
                logger.info(f"Lower threshold alert for {metric_name}: {metric_value} < {metric_thresholds['lower']}")
                alert_triggered = True
